...
Function Invoke-DownloadAndInstallPETools() {
  $install_args = @("/qn", "/norestart", "/i", 'https://s3.amazonaws.com/pe-client-tools-releases/2017.2/pe-client-tools/17.2.3/repos/windows/pe-client-tools-17.2.3-x64.msi')
  Write-Host "Installing PE Client Tools. Running msiexec.exe $install_args"
  $process = Start-Process -FilePath msiexec.exe -ArgumentList $install_args -Wait -PassThru
  if ($process.ExitCode -ne 0) {
    Write-Host "Installer failed."
    Exit 1
  }
  Write-Host "PE Client Tools successfully installed."
}
...
if (-not ($env:Path.Contains("C:\Program Files\Puppet Labs\Client\bin"))) {
  $env:Path += ";$PEClientToolsBinPath"
}
...
if ($PuppetMaster -eq '') {
  ...
  Write-Host '4. Download & Install PE Client Tools (17.2.3 windows)'
  Write-Host ''
  do {
  ...
  switch ($option) {
    ...
    '4' { Invoke-DownloadAndInstallPeTools }
  }
} else {
