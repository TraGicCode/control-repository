    ...
    stage("Test To Development"){
      ...
      steps {
        script {
          ...
          def jobResult = sh(returnStdout: true, script: 'puppet job run --query \\"inventory[certname] { trusted.extensions.pp_environment = \\'development\\' and nodes { deactivated is null } }\\" --noop --format json')
          echo jobResult
        }
